...
- “NEEDS_MORE_INFO” (lacks necessary detail or specificity),
...
Q: “Why is my API returning a 500 error?”  
A: NEEDS_MORE_INFO
...
export async function improvePrompt(rawPrompt) {
  let promptStatus = await runOllama({ model: 'llama3.2:8b', prompt: classifyPrompt(rawPrompt)});
  ...
  console.log('-----------------------------------');
  const rawOutput = await runOllama({ model: 'llama3.2:8b', prompt: template });
...
(async () => {
  const raw = 'What is __name__ in python';
